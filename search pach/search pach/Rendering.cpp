...
void drawArrow(RenderWindow& window, float x1, float y1, float x2, float y2, Color color, float offsetX, float offsetY) {
    x1 += offsetX;
    y1 += offsetY;
    x2 += offsetX;
    y2 += offsetY;
...
void createNodes(std::vector<CircleShape>& smallCircles, std::vector<Text>& labels, const Vector2f& center, int count, float radius, float smallCircleRadius, const Font& font) {
